...
class State_controller:
    ...
    def add_new_state(self, round_=-1, agg_gradient="", base_result=""):
        state_data = state(round_=round_,
                           agg_gradient=agg_gradient,
                           base_result=base_result)

        self.states.append(eval(state_data.json()))

    def add_update(self, round_, cid, gradient):
        if not round_ == self.get_last_round():
            return
        self.append_incoming_gradient({"gradient": gradient, "cid": cid})
        ...
        if len(self.states) >= self.max_iteration and self.get_last_round() >= self.max_iteration - 1:
            if not self.is_saved:
                ...
                if int(os.getenv("ID")) == 0:
                    import torch
                    if not os.path.exists("/root/app/save_models/"):
                        os.mkdir("/root/app/save_models/")

                    for i in states_:
                        model_save = "/root/app/save_models/round_{}.pt".format(i["round"])
                        ...
                        for j in i["incoming_gradient"]:
                            model_save = "/root/app/save_models/round_{}_cid_{}.pt".format(i["round"], j["cid"])
                ...
                # save json report
                result = {"data": states_}
                with open('/root/app/{}_round_result_{}.json'.format(self.max_iteration, os.getenv("ID")),
                          'w') as outfile:
            ...
            # Done
            if int(os.getenv("ID")) == 0:
                open("/root/app/save/Done", 'a').close()
            return False
        else:
            return True

    def save_round_point(self, round_):
        if not os.path.exists("/root/app/save/"):
            os.mkdir("/root/app/save/")
        open("/root/app/save/round_{}".format(round_), 'a').close()
